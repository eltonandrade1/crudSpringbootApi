...
import com.elton.springboot.config.tokenenhancer.CustomTokenEnhancer;
...
public class AuthorizationServerConfig extends AuthorizationServerConfigurerAdapter {
	...
	public void configure(ClientDetailsServiceConfigurer clients) throws Exception {
		clients.inMemory() // armazena em memória, mas poderia ser banco
				...
				.scopes("read", "write") // permissões de acesso para aplicaçao cliente (mesmo um usuário admin nessa
											// aplicaçao somente ler e escreve)
				.authorizedGrantTypes("password", "refresh_token") // será utilzada senha informado pelo usuario na
																	// aplicação angular e autenticação via token
				.accessTokenValiditySeconds(1800) // configura o tempo que o token fica ativo expresso em segundos
				.refreshTokenValiditySeconds(3600 * 24) // configura a validade do refresh token para 24 horas (3600seg
														// = 1 hora * 24 = 1 dia)
				.and().withClient("mobile") // cliente mobile
				.secret("m0b1l30").scopes("read") // permissões de acesso para aplicaçao mobile (mesmo um usuário admin
													// nessa aplicaçao somente ler)
	...
	public void configure(AuthorizationServerEndpointsConfigurer endpoints) throws Exception {
		...
		endpoints.tokenStore(tokenStore()).tokenEnhancer(tokenEnhancerChain).reuseRefreshTokens(false) // após a
																										// expiraçao no
																										// token será
																										// gerado um
																										// novo e não
	...
	/**
	 * Método responsável por adicionar informações do usuário logado no sistema 
	 * ao token jwt
	 * 
